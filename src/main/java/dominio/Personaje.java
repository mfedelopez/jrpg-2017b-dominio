...
public abstract class Personaje extends MadreDeTodo implements Peleable, Serializable {
  ...
  /**
     ...
     * @param itemAPoner items a recibir.
     * @param itemASacar items a eliminar.
     */
    
  public void trueque(final ArrayList<Item> misItems,
            final ArrayList<Item> itemAPoner, final DefaultListModel<String> itemASacar) {
    int j = 0;
    boolean loop = true;
    ArrayList<Item> aux = misItems;
    while (itemASacar.size() > 0) {
      while (loop) {
        if (misItems.get(j).getNombre().equals(itemASacar.get(0))) {
          itemASacar.remove(0);
